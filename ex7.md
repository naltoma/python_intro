...
- 今後の演習では、<span style="color:red">指定がない限りはインタプリタで開発するか、ファイル編集で開発するかは好きに選んで</span>もらって構いません。同様に、<span style="color:red">関数を作れという指示がない場合でも、自由に作成OK</span>です。
...
- 概要
  ...
  - 今回処理するCSVファイルとして
  [score.csv](???)を用意した。演習7.1以下ではこのファイルをダウンロードして利用すること。
